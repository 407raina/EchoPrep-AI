...
export class RealtimeChat {
  ...
  async init() {
    try {
      ...
      const tokenData = await apiFetch<any>("/api/ai/realtime-token", {
        method: "POST",
        body: JSON.stringify({ instructions: this.instructions }),
      });

      const ephemeral = tokenData?.client_secret?.value as string | undefined;
      ...
      const model = import.meta.env.VITE_REALTIME_MODEL ?? "gpt-4o-realtime-preview-2024-10-01";
      ...
      if (!answerSdp || answerSdp.trim().length === 0) {
        throw new Error("Received empty SDP response from OpenAI");
      }
